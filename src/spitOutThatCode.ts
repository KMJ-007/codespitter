...
import { generatePdf } from './genreatePdf';
                ...
                const contents = files.map(file => {
                    try {
                        ...
                        if (fileExtension === '.html') {
                            const textCode = `<pre>${fileContent}</pre>`; // wrap all html files in <pre> tags
                        ...
                        return `<pre>${fileContent}</pre>`; // wrap all other files in <pre> tags
                    } catch (error) {
                        vscode.window.showErrorMessage(`This file is not good for my stomach skipping it: ${file.fsPath}`);
                        return '';
                    }
                }).filter(content => 
                    // also need to remove the file which don't have text content or have unicode content
                    content !== undefined &&
                    content !== null &&
                    content !== '' &&
                    !content.includes('�')
